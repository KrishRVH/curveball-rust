...
├── assets/sounds/*.wav                            # 5 files from the reference kit
├── src/
...
- Title: "curveball" (white) at (60.3, 32.25); buttons "start game" (174.5, 115.1), "high scores"
  (175.5, 132.3), and the Rust-only "zen" extension below them (D9) — white 64 × 11.3 pill,
  black ~7 px label, centered.
...
| M6 | Fidelity audit (§15) | Checklist complete; `DEVIATIONS.md` finalized with intentional D1-D11 tradeoffs |
...
5. Quirk demonstrations: Q2 (click pop), Q4 (pip counts), Q6 (paddle persistence across splash),
   ...
   (render interpolation smooths high-refresh displays without mutating gameplay), and D11
   (experimental sim-rate override is clearly non-faithful).
6. Close `DEVIATIONS.md`: it must contain only approved fidelity or product-quality deviations
   (currently D1-D11) plus implementation corrections C1-C4.
