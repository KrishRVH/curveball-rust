...
| Modern sounds (generated) | `assets/sounds/modern/*.wav` | D13 opt-in 48 kHz recreations generated by `reference/kit/recreate_sounds.py` |
...
Faithful mode embeds these files as the default. Modern mode (D13) uses generated 48 kHz mono
recreations with matched duration/RMS/source-resampled peaks, the same trigger mapping, and only
restrained high-band reconstruction above the source Nyquist limit.
...
├── assets/sounds/*.wav                            # 5 faithful files from the reference kit
├── assets/sounds/modern/*.wav                     # 5 generated D13 modern recreations
├── src/
...
## 8. Audio
Embed both the 5 faithful WAVs and the 5 generated Modern WAVs with `include_bytes!`. Default builds
use `rodio` for sound output while keeping macroquad graphics-only, avoiding `quad-snd` startup
panics on Linux hosts without a usable PCM route. The runtime decodes both embedded sound sets once
during audio startup. Triggers exactly as in §3.8 / §4.4 / §4.5: one playback per event, overlapping
instances allowed (Flash `Sound.start(0, 1)` semantics), at master volume 0.8. If device setup or
decode fails, log and continue silently.
...
Regenerate the Modern set with:
...
- Title: "curveball" (white) at (60.3, 32.25); buttons "start game" (174.5, 115.1), "high scores"
  (175.5, 132.3), and Rust-only extensions "zen" (D9), `SOUND: ...` (D13), and `VISUAL: ...` (D14)
  below them — white pill buttons, black ~7 px labels, centered.
